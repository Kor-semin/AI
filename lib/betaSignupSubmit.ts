 ...
 * 베타 신청 제출.
 *
 * - `NEXT_PUBLIC_BETA_SIGNUP_ENDPOINT`가 비어 있으면: `console.info` + 데모용 `alert`만 (서버 전송 없음).
...
export async function submitBetaSignup(payload: BetaSignupPayload): Promise<BetaSignupResult> {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.warn("[beta signup] POST failed", res.status, body);
    ...
    console.warn("[beta signup] fetch error", e);
    if (typeof window !== "undefined") {
      window.alert("신청 저장에 실패했습니다. 잠시 후 다시 시도해주세요.");
    }
