  ...
  return (
    <div className="min-h-screen bg-[#0A0B0D] p-5 sm:p-6 xl:p-8">
      ...
      <section className="mt-6 rounded-2xl border border-[#2B3037] bg-[#14171B] p-5 sm:p-6" aria-labelledby="followup-list-title">
        <div className="flex flex-wrap items-center justify-between gap-4"><h2 id="followup-list-title" className="text-lg font-semibold text-[#F4F6F8]">Follow-up 목록</h2><span className="text-[10px] text-[#7F8792]">Demo data · 2026. 06. 22 기준 예시</span></div>
        <div className="mt-5 overflow-x-auto"><div className="min-w-[760px]">
          ...
          <div className="mt-3 space-y-2">{displayRows.map(({ followUp, displayStatus }, index) => (
            <div key={followUp.id} className="grid min-h-[72px] grid-cols-[.8fr_1.25fr_1fr_.8fr_1.5fr_.8fr] items-center gap-4 rounded-lg border border-[#2B3037] bg-[#1A1E23] px-4 text-[11px]">
              <span className="font-semibold text-[#F4F6F8]">{customerLabels[followUp.customerId]}</span><span className="text-[#B7BDC6]">{["Genesis GV70", "BMW X5", "Mercedes-Benz E300", "Audi A6"][index]}</span><span className="text-[#7F8792]">{index === 0 ? "예시 · 14:00" : index === 1 ? "예시 · 6월 20일" : index === 2 ? "예시 · 6월 21일" : "예시 · 6월 24일"}</span><span className={displayStatus === "overdue" ? "font-medium text-[#B48A48]" : displayStatus === "completed" ? "text-[#4E8A66]" : "text-[#B7BDC6]"}>{statusLabels[displayStatus]}</span><span className="text-[#B7BDC6]">{followUp.purpose}</span><span className="text-[#7F8792]">{displayStatus === "completed" ? "확인 완료 예시" : "구조 확인"}</span>
            </div>
          ))}</div>
        </div></div>
        <p className="mt-5 rounded-lg border border-[#2B3037] bg-[#1A1E23] px-4 py-3 text-[10px] leading-4 text-[#7F8792]">실제 운영 데이터가 아닙니다. 완료 처리와 일정 저장 기능은 연결하지 않았으며, 표시는 dueDate/status/today 계산 결과의 미리보기입니다.</p>
      </section>
    </div>
  );
}
