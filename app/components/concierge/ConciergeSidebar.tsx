...
function navButtonClass(active: boolean): string {
  return [
    ...
    active
      ? "bg-sky-500/12 pl-[calc(12px+0.375rem)] pr-3 text-[#F8FAFC] ring-2 ring-inset ring-sky-400/25 before:absolute before:left-[3px] before:top-2 before:bottom-2 before:w-[3px] before:rounded-full before:bg-sky-400/80"
      : "px-3.5 text-[#E5E7EB] hover:bg-white/[0.06] hover:text-white",
  ].join(" ");
}
  ...
  return (
    <>
      ...
      <nav
        ...
        aria-label="업무 영역 메뉴"
      >
        <div className="rounded-2xl border border-white/[0.1] bg-slate-900/55 px-1.5 py-1.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] backdrop-blur-md">
                ...
                <button
                  ...
                  className={[
                    ...
                    active
                      ...
                      : "border-white/[0.1] bg-slate-950/35 text-slate-100 hover:border-white/20 hover:bg-slate-900/65",
                  ].join(" ")}
                  aria-current={active ? "page" : undefined}
                  aria-label={`${title}, ${subtitle}`}
                >
      ...
      <aside className="hidden w-[274px] shrink-0 lg:block">
        <div className="sticky top-24 flex min-h-[calc(100vh-8rem)] flex-col rounded-[18px] border border-white/[0.1] bg-[#07111f]/95 pb-6 pt-5 shadow-[0_28px_64px_-24px_rgba(0,0,0,0.55)] backdrop-blur-md">
          <button
            type="button"
