      ...
      >
        <div className="flex items-start justify-between gap-3 border-b border-white/[0.09] px-4 pb-4 pt-[max(12px,calc(env(safe-area-inset-top,0px)+10px))] sm:px-5 sm:pt-5">
          <div className="min-w-0">
            <h2 id="app-preview-toc-title" className="text-[17px] font-semibold leading-snug tracking-tight text-white sm:text-lg">
              {t("preview.toc.title")}
            </h2>
            <p className="mt-1 text-[13px] font-medium leading-relaxed text-slate-400 sm:text-[14px]">{t("preview.toc.subtitle")}</p>
          </div>
          <button
            type="button"
            className="shrink-0 rounded-xl border border-white/[0.14] bg-white/[0.06] px-3 py-2 text-[13px] font-semibold text-slate-200 transition hover:bg-white/[0.11] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-400/35 touch-manipulation"
            onClick={onClose}
          >
            {t("preview.toc.close")}
          </button>
            ...
            {PREVIEW_ITEMS.map((item) => (
              <li key={item.section}>
                <button
                  ...
                  className="w-full min-h-[4.5rem] rounded-[14px] border border-white/[0.12] bg-white/[0.05] px-4 py-[0.875rem] text-left shadow-[inset_0_1px_0_rgba(255,255,255,0.06)] ring-1 ring-inset ring-sky-400/06 transition-[border-color,box-shadow,transform] hover:border-sky-400/38 hover:bg-white/[0.08] hover:shadow-[0_0_40px_-12px_rgba(56,189,248,0.15)] motion-safe:active:scale-[0.994] motion-reduce:transition-none focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-400/42 touch-manipulation sm:min-h-[4.65rem]"
                >
                  <span className="block text-[15px] font-semibold text-slate-50">{t(item.titleKey)}</span>
                  <span className="mt-1 block text-[13px] leading-relaxed text-slate-400">{t(item.descKey)}</span>
                </button>
              </li>
            ))}
            <li>
              <Link
                href={JOIN_PATH}
                prefetch={false}
                className="block w-full rounded-[14px] border border-violet-400/28 bg-gradient-to-br from-violet-500/[0.1] to-white/[0.04] px-4 py-[0.875rem] text-left shadow-[inset_0_1px_0_rgba(255,255,255,0.06)] ring-1 ring-inset ring-violet-400/15 transition hover:border-violet-400/42 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-400/35 touch-manipulation min-[390px]:min-h-[76px]"
                onClick={() => {
                  try {
                    onClose();
                  } catch {
              ...
              >
                <span className="block text-[15px] font-semibold text-slate-50">{t("preview.toc.betaTitle")}</span>
                <span className="mt-1 block text-[13px] leading-relaxed text-slate-400">{t("preview.toc.betaDesc")}</span>
              </Link>
            </li>
          </ul>

          <div
            className="rounded-xl border border-white/[0.08] bg-black/25 px-3 py-2.5 text-[11px] leading-relaxed text-slate-500 sm:text-[12px]"
            role="note"
          >
        ...
        <div className="flex flex-col gap-2.5 border-t border-white/[0.09] px-4 py-[max(12px,calc(env(safe-area-inset-bottom,0px)+12px))] sm:flex-row sm:flex-wrap sm:justify-end sm:gap-3 sm:px-5 sm:py-4">
          <button
            type="button"
            className="sensora-premium-primary-workspace inline-flex min-h-[48px] w-full shrink-0 items-center justify-center rounded-xl px-5 py-3 text-[14px] font-semibold tracking-tight sm:min-h-[46px] sm:w-auto touch-manipulation"
          ...
          <button
            type="button"
            className="inline-flex min-h-[44px] w-full shrink-0 items-center justify-center rounded-xl px-5 py-2.5 text-[13px] font-semibold text-slate-400 hover:text-slate-200 sm:w-auto touch-manipulation"
            onClick={onClose}
          >
            {t("preview.toc.close")}
          </button>
