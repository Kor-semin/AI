...
export function SensoraGuideImageViewer({
  ...
  return (
    ...
    >
      ...
      />
      <div
        className="landing-guide-dialog-animate sensora-guide-modal-cosmos relative z-[1] flex max-h-[min(94dvh,94vh)] w-full max-w-[min(96vw,min(1320px,100%))] flex-col overflow-hidden rounded-t-[22px] border border-white/[0.14] bg-gradient-to-b from-[#0a1628]/98 to-[#07111f]/97 pb-[max(4px,env(safe-area-inset-bottom,0px))] shadow-[0_40px_100px_-28px_rgba(0,0,0,0.72),0_0_0_1px_rgba(255,255,255,0.045)_inset,0_0_60px_-24px_rgba(56,189,248,0.065)] backdrop-blur-2xl sm:max-h-[min(92dvh,92vh)] sm:rounded-2xl sm:pb-0"
      ...
      >
        ...
        <div className="flex min-h-0 min-w-0 flex-1 flex-col gap-2 overflow-hidden p-3 sm:gap-3 sm:p-5">
          <div className="relative flex min-h-0 flex-1 items-center justify-center">
            ...
            <div className="absolute inset-y-0 right-1 z-[2] flex items-center sm:right-2">
              <button
                type="button"
                onClick={goNext}
                className="sensora-guide-nav-fab flex size-11 items-center justify-center rounded-full border border-white/[0.14] bg-[#0c1422]/95 text-sky-50 shadow-[inset_0_1px_0_rgba(255,255,255,0.06),0_12px_32px_-8px_rgba(0,0,0,0.55)] backdrop-blur-md transition duration-[220ms] hover:border-sky-400/38 hover:bg-[#111d2f] hover:shadow-[0_0_28px_-8px_rgba(56,189,248,0.18)] active:scale-[0.96] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-400/45 motion-reduce:transition-none touch-manipulation"
            ...
            <div className="relative z-[1] mx-auto flex h-[min(78dvh,880px)] w-full max-w-[min(1400px,96vw)] min-h-[min(200px,40dvh)] items-center justify-center px-11 sm:px-14">
              {broken[safeIndex] ? (
                ...
                <Image
                  ...
                  sizes="(max-width:640px) 96vw, (max-width:1280px) 92vw, min(1400px, 94vw)"
                  quality={100}
          ...
          <p className="px-1 text-center text-[11px] font-medium leading-snug text-slate-400 sm:text-xs">{captionLabel}</p>

          <div className="sensora-guide-thumb-rail flex max-w-full shrink-0 flex-row gap-2 overflow-x-auto overflow-y-hidden pb-[max(4px,env(safe-area-inset-bottom,0px))] sm:pb-0">
            {images.map((item, i) => {
              ...
              return (
                <button
                  ...
                  onClick={() => setIndex(i)}
                  className={[
                    "relative h-16 w-[4.85rem] shrink-0 overflow-hidden rounded-lg border shadow-[0_10px_28px_-16px_rgba(0,0,0,0.45)] transition-[border-color,opacity,transform,box-shadow] duration-[220ms] ease-[cubic-bezier(0.22,1,0.32,1)] motion-reduce:transition-none touch-manipulation",
                    active
