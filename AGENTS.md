...
## 11. Sensora 최종 제품·디자인 기준 (고정)

이 절은 Sensora의 장기 제품 방향을 고정하기 위한 **최종 기준**입니다.  
랜딩/앱/관리자 화면 작업, 카피 수정, QA 검증, Cursor 요청서 작성 시 **항상 우선 적용**합니다.

### 제품 정체성

- Sensora는 단순 AI 데모가 아니라 **프리미엄 자동차 영업 CRM**입니다.
- AI는 전면 브랜드가 아니라 **내부 보조 엔진**입니다.
- 사용자가 느껴야 하는 가치는 자동화 과장이 아니라, **상담 정리·다음 연락·고객 이력·사후관리 정확도 향상**입니다.
- 장기적으로 기존 CRM을 **대체/보완**할 수 있도록, MVP 단계부터 구조·품질·신뢰를 우선합니다.
...
- **사용자 확인 후 저장**
- AI는 **정리/제안**만 수행
- 최종 판단·고객 응대·발송은 **영업사원이 직접 수행**

금지 표현 예시:

- AI가 다 해준다 / 자동으로 처리한다 / 추천만 믿으면 된다 / 챗봇처럼 응답한다
...
- 과한 네온/블루 중심 SaaS
- 개발자 테스트 페이지 느낌
- 구 다크 네이비 스타일 + 버건디 스타일 혼합
- 명암/대비 불일치
- 모바일에서 읽기 어려운 과한 헤드라인/중첩 배경
- 앱 화면 전체를 버건디로 과다 도포
- 카페/웨딩/뷰티처럼 과도하게 부드러운 분위기
...
- 베이지: 보조 배경·카드
- 딥 브라운: 텍스트·라인·구조
- 앱/관리자: 버건디는 **포인트 위주**로 제한
- 대비가 부족하면 즉시 조정
- 구 스타일(다크 네이비)과 신 스타일(버건디) 혼합 금지

### 화면별 고정 기준

#### 랜딩

- 3초 안에 서비스 이해 + 무료 베타 신청 CTA 명확화
- 흐름: 문제 제기 → 해결 흐름 → AI 상담 정리 예시 → 신뢰 원칙 → 무료 베타 신청
- 브랜드 톤은 강하게 보여도 과장된 AI 인상은 금지

#### 앱

- 매일 쓰는 영업 업무 도구처럼 보여야 함
- 가독성·입력 편의성·업무 효율 우선
- 첫 화면 핵심: 오늘 할 일 / 빠른 상담 메모 / AI 정리 결과 / 문자 초안 복사 / 다음 연락 저장
- 기능 개수보다 업무 흐름 명확성이 우선

#### 관리자

- 보안·마스킹·권한 검증·상태코드 명확화 우선
- 목록은 기본 마스킹, 권한 미충족 시 PII 미노출
- 상태코드 기준 유지:
  - `503`: 운영 env 미설정
  - `401`: 미로그인
  - `403`: 관리자 아님
- 화려함보다 정보 가독성과 보안 신뢰 우선

### 작업 강제 규칙 (랜딩/앱/관리자 공통)

- 랜딩/앱/관리자 화면 관련 작업 전후에 아래 문서를 **반드시** 확인합니다.
  - `docs/sensora-production-setup.md`
  - `docs/sensora-design-qa-checklist.md`
- 디자인/카피/UX 변경 시 `docs/sensora-design-qa-checklist.md` 항목을 기준으로 자체 검증합니다.
- “대충 만든 데모 느낌” 요소(불균형 카드/버튼/대비/카피)는 즉시 제거합니다.
